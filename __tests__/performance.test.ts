...
describe('VufForm Performance Tests', () => {
	describe('Large Form Performance', () => {
		...
		test('getJson performance for large form', () => {
			...
			console.log(
				`Average getJson execution time: ${averageTime.toFixed(3)}ms`,
			);
		...
		test('groupIsValid performance for large form', () => {
			...
			console.log(
				`Average groupIsValid execution time: ${averageTime.toFixed(3)}ms`,
			);
	...
	describe('Nested Form Performance', () => {
		...
		test('getJson performance for nested form', () => {
			...
			console.log(
				`Average getJson execution time for nested form: ${averageTime.toFixed(3)}ms`,
			);
	...
	describe('Memory Usage Tests', () => {
		test('memory usage for creating many forms', () => {
			...
			const endTime = performance.now();
			console.log(
				`Time to create 1000 large forms: ${(endTime - startTime).toFixed(3)}ms`,
			);
	...
	describe('Stress Tests', () => {
		test('rapid form data setting', () => {
			...
			console.log(
				`Average time per update: ${(totalTime / 1000).toFixed(3)}ms`,
			);
