...
export class VufForm<T extends Record<string, FieldConfig<any>>> {
	...
	// Solid 2のsignal writeは次のmicrotaskまでstagedされる。isErrorField/
	// groupIsValidの同期boolean契約を保つため、検証開始状態はplain値でも保持する。
	private _validationStarted = false;
		...
		for (const key in this._fields) {
			if (!key.includes('$')) {
				...
				createEffect(
					() => fieldSignal[0](),
					(value) =>
						this.attachNestedWatch(value, isValidateImmediately, owner),
				);
			}
		}
	}
	...
	private ensureNestedWatched(
		...
		const run = () => {
			...
			if (this._validationStarted) form.startValid();
		};
